...
class Preprocessor:
    ...
    def process_record(self, record: Record) -> Optional[Record]:
        ...
        try:
            ...
            if not validate_record(processed_record.to_dict(), preprocessing_schema):
                logging.warning(f"Preprocessed record ID {processed_record.id} failed validation.")
                return None
            ...
            if not processed_data:
                logging.warning(f"Record ID {processed_record.id} was not processed successfully by {self.config['provider']}.")
                return None
            ...
            try:
                ...
                if not processed_record_new:
                    logging.error(f"Processed data for record ID {processed_record.id} is invalid.")
                    return None
            except json.JSONDecodeError as e:
                logging.error(f"JSON decoding error for record ID {processed_record.id}: {e}")
        ...
        try:
            ...
            for idx, raw_record in enumerate(records_raw, start=1):
                ...
                if not processed_record:
                    logging.warning(f"Record ID {record.id} failed processing and will be skipped.")
                ...
                logging.info(f"Record ID {processed_record.id} processed and saved successfully.")
            ...
            logging.error(f"An error occurred while processing all records: {e}")
    ...
    def load_record(self, raw_input: str, formatted: bool = True) -> Optional[Record]:
        ...
        try:
            if formatted:
                ...
                logging.error("Failed to parse formatted record. Ensure it is valid JSON or tagged text.")
                return None
            else:
                # Treat as unformatted text
                record = Record.from_unformatted_text(raw_input, self.llm_client.format_text)
                if record:
                    logging.debug("Record loaded from unformatted text via LLM.")
                    return record

                logging.error("Failed to load unformatted record after LLM processing.")
                return None
